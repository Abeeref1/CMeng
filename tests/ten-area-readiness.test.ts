...
async function schedule(store:any,date:string,name:string,finish?:string,bad=false){return store.ingestEvidenceFile({projectId:store.listProjectIds()[0]!,bytes:Buffer.from(xer(date,finish,bad)),mediaType:'text/plain',sourceFilename:name,uploadedAt:stamp,uploadIntent:'replace_current_basis',scheduleRole:'update'});}
...
test('3 explicit adoption controls current authority regardless of filename or data-date order',async t=>{const {store,state}=fixture(t);await schedule(store,'2031-07-01','baseline-1990.xer');await schedule(store,'2031-05-01','latest-2099.xer');await schedule(store,'2031-06-01','rev99.xer');assert.equal(projectControlSchedule(state)?.revision.model.dataDateIso?.slice(0,10),'2031-06-01');assert.ok(state.schedules.every(s=>s.role==='update'));});
