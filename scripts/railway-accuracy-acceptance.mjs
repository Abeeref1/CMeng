...
try {
  ...
  check('Claim resolver receives semantic signals from governed evidence', resolverStages?.signalBearingEventCount > 0);
  check('Scored-OR retrieval produces candidates before confidence gates', resolverStages?.retrievedCandidateCount > 0);
  check('Deterministic prefilter produces bounded candidates without weakening gates', resolverStages?.preFilterCandidateCount > 0 && resolverStages?.boundedCandidateCount > 0);
