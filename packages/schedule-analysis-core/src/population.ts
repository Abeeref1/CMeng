...
export function numericDistribution(values: readonly (number | null)[]) {
  ...
  const maximumCount = maximum === null ? null : counts.get(Number(maximum.toFixed(6)))!;
  return { totalCount: values.length, knownCount: known.length, unknownCount: values.length - known.length,
    ...
    maximumPercent: maximumCount === null ? null : 100 * maximumCount / known.length, groups: groups.slice(0,10),
    dominantValue: groups[0]?.value ?? null, dominantCount: groups[0]?.count ?? null,
