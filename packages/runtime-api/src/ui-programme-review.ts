...
function renderProgrammeReview(review){
  ...
  return '<section class="notice warn programme-review" aria-label="Programme adoption review"><b>'+(review.state==='missing'?'Programme adoption required':'Programmes awaiting review')+'</b><p>'+escapeHtml(review.explanation)+'</p>'+(pending.length?'<p>'+pending.length+' programme'+(pending.length===1?' is':'s are')+' awaiting adoption. '+(review.state==='missing'?'Adopt a reviewed programme to enable reporting.':'Reporting continues to use the adopted programme until you select a replacement.')+'</p><div class="table-wrap"><table><thead><tr><th>Uploaded programme</th><th>Data Date</th><th>Role</th><th>Decision</th></tr></thead><tbody>'+rows+'</tbody></table></div>':'')+'<p><button class="btn small programme-review-documents">Open programme documents</button></p><p class="programme-review-message" role="status"></p></section>';
}
function bindProgrammeReview(container){
  ...
  container.querySelectorAll('.programme-review-adopt').forEach(button=>button.onclick=async()=>{
    ...
    try{await api('/api/projects/'+encodeURIComponent(owner)+'/schedule/revisions/'+encodeURIComponent(revision)+'/adopt',{method:'POST'});if(project()===owner){await refresh(false);const job=typeof projectUploadJobs!=='undefined'?projectUploadJobs.get(owner):null;if(job?.state==='complete'){const review=currentModuleResult?.scheduleAuthorityReview||currentModuleResult?.data?.scheduleAuthorityReview;job.message=review?.pendingSchedules?.length?'Programme adopted · other revisions await review':'Programme adopted · project position updated';renderBackgroundUploads();}}}
