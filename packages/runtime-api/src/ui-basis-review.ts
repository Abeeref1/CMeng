...
function renderCommercialExceptions(position,key,data){
  ...
  if(['contract-particulars-bonds','commercial-claims-notices'].includes(key)){
    ...
    if(rules.length)html+=basisPanel('Contract notice periods by version','The original and amended rules are separate. Notice dates alone cannot establish the applicable trigger date.',
      basisTable(['Requirement','Days','Effective from','Replaced from','Trigger','Source'],rules.map(r=>[r.noticeKind==='detailed_claim'?'Fully detailed claim':'Initial claim notice',r.noticePeriodDays,r.effectiveFromIso||'Original / date not stated',r.effectiveToIso||'No later version supplied',humanizeKey(r.triggerBasis||'not_stated'),(r.evidenceRefs||[]).map(e=>e.sourceId+' '+e.locator).slice(0,2).join('; ')]))+
...
function renderBasisReviews(data,key){
  ...
  if(s){
    html+=basisPanel(fmt(s.deadline.lateCount)+' of '+fmt(s.packageCount)+' package finishes fall after the contract date',s.interpretation,
      '<p>Contract finish '+escapeHtml(planningShortDate(s.contractFinishIso))+'. '+fmt(s.deadline.positiveFloatButLateCount)+' late package finishes still have positive submitted float. Dated lateness ranges from '+fmt(s.deadline.latenessDays.min)+' to '+fmt(s.deadline.latenessDays.max)+' calendar days. Submitted criticality is not a contract-compliance test.</p>'+
