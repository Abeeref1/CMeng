...
function experienceCertificateChart(rows,currency,future=false,dataDate=null,basis='source_row_sum_only'){
  ...
  const ticks=Array.from({length:5},(_,i)=>{const v=ymin+(ymax-ymin)*i/4,yy=y(v);return '<line x1="'+left+'" x2="'+(width-right)+'" y1="'+yy+'" y2="'+yy+'" stroke="#dce4ed"/><text x="'+(left-10)+'" y="'+(yy+4)+'" text-anchor="end">'+(v/1e6).toFixed(1)+'</text>'+(canSum?'<text x="'+(width-right+12)+'" y="'+(yy+4)+'">'+((cmin+(cmax-cmin)*i/4)/1e6).toFixed(1)+'</text>':'');}).join('');
