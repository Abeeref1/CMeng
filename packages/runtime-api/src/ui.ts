...
function renderManagementMetricGrid(metrics){
  ...
  return '<div class="management-metric-grid">'+metrics.map(m=>{
    const value=m.value===null||m.value===undefined?"Not established":fmt(m.value)+(m.unit?" "+m.unit:"");
    return '<article class="management-metric-card '+escapeHtml(m.health||"unavailable")+'"><div class="management-metric-head"><span>'+escapeHtml(m.label)+'</span>'+managementHealthBadge(m.health)+'</div><strong>'+escapeHtml(value)+'</strong><div class="management-metric-badges">'+managementAuthorityBadge(m.state)+managementAuthorityBadge(m.authority)+'</div><small><b>Basis:</b> '+escapeHtml(m.basis||"Not established")+'</small>'+(m.consequence?'<p><b>Consequence:</b> '+escapeHtml(m.consequence)+'</p>':'')+(m.action?'<p><b>Action:</b> '+escapeHtml(m.action)+'</p>':'')+'<div class="management-metric-owner">'+managementModuleLink(m.owningModule)+'</div></article>';
  }).join("")+'</div>';
}
