  ...
  if(performance){
    ...
    if(key==="cash-flow"){
      const cashSections=(performance.cashFlow?.currencies||[]).map(row=>{
        ...
        const hasCurve=plottedPointCount>=2;
        ...
        const netReady=netValue!==null;
        const missingCore=[
          paidValue===null?"dated paid cash receipts":null,
          actualValue===null?"dated actual expenditure":null
        ].filter(Boolean);
        const statusLabelFor=finding=>metricValue(finding)!==null?"Established":finding?.state==="partial"?"Partial":finding?.state==="conflicted"?"Conflicted":"Not established";
        const readinessItem=(label,finding,why,owner)=>{
          const ready=metricValue(finding)!==null;
          const tone=ready?"ready":finding?.state==="conflicted"?"blocked":"partial";
          return '<div class="cash-readiness-item '+(ready?"established":"missing")+'"><div><span>'+escapeHtml(label)+'</span><b>'+escapeHtml(statusLabelFor(finding))+'</b></div><span class="badge '+tone+'">'+escapeHtml(ready?"Usable":"Required")+'</span><small>'+escapeHtml(why)+'</small>'+managementModuleLink(owner,ready?"Open source":"Resolve source")+'</div>';
        };
        const hero=netReady
          ? '<div class="cash-flow-hero established"><div><span>Current evidenced net cash</span><strong>'+escapeHtml(fmt(netValue)+" "+row.currency)+'</strong><p>Paid income less actual expenditure. Certification is not treated as cash.</p></div><div class="cash-flow-hero-stats"><div><span>Peak funding need</span><b>'+escapeHtml(peakValue===null?"Not established":fmt(peakValue)+" "+row.currency)+'</b></div><div><span>Paid cash</span><b>'+escapeHtml(fmt(paidValue)+" "+row.currency)+'</b></div><div><span>Actual expenditure</span><b>'+escapeHtml(fmt(actualValue)+" "+row.currency)+'</b></div><div><span>Certified unpaid</span><b>'+escapeHtml(unpaidValue===null?"Not established":fmt(unpaidValue)+" "+row.currency)+'</b></div></div></div>'
          : '<div class="cash-flow-hero withheld"><div><span>Current cash position</span><strong>Withheld</strong><p>CMeng cannot defend a net cash position yet because '+escapeHtml(missingCore.join(" and ")||"the required dated cash series is incomplete")+'. Missing cash is not treated as zero.</p></div><div class="cash-flow-hero-rule"><b>What is still valid?</b><p>Certification, payment-register and cost evidence remain visible in their owning modules, but they do not become cash unless the dated cash basis is established.</p></div></div>';
        const curve=hasCurve
          ...
          : '<div class="cash-flow-curve-withheld"><div><span>Cash-flow curve</span><b>Not plotted</b></div><p>A management S-curve requires at least two defensible dated points. CMeng will not draw a flat or zero curve from missing evidence.</p></div>';
        const secondary=[];
        ...
        return '<section class="planning-panel primary cash-flow-position"><div class="planning-panel-head"><div><h4>'+escapeHtml(row.currency)+' · Cash Flow & Funding</h4><p>Only cash-supported positions appear as cash. Certification, expenditure plans and missing evidence remain separate.</p></div><span class="badge '+(netReady?"ready":"partial")+'">'+escapeHtml(netReady?"Cash established":"Cash incomplete")+'</span></div><div class="planning-panel-body">'+
          ...
          '<div class="cash-readiness-grid">'+
            readinessItem("Paid cash receipts",row.paidIncome,"Required for net cash and funding need.","payments")+
            readinessItem("Actual expenditure",row.actualExpenditure,"Required for net cash and funding need.","cost-forecast")+
            readinessItem("Certified income",row.certifiedIncome,"Supporting certification position; not cash by itself.","payments")+
            readinessItem("Expenditure forecast",row.expenditureForecast,"Forward expenditure basis; not actual cash.","cost-forecast")+
          '</div>'+
          curve+
