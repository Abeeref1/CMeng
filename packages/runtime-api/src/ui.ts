        ...
        <section class="card">
          ...
          <div class="upload-row">
            <div class="upload-box">
              ...
              <small>Select baseline, multiple updates, revised baseline and recovery files together. Assign the role per file before upload.</small>
            ...
            <div class="upload-box">
              ...
              <small>Select one or more BOQ revisions. CSV, Excel and PDF are accepted and retained as separate evidence.</small>
            ...
            <div class="upload-box">
              ...
              <small>Select the main contract, amendments and appendices together. Their roles remain separate and amendments do not silently replace the base contract.</small>
              <input type="file" id="contractFiles" multiple accept=".pdf,.docx">
          ...
          <div class="wide-upload">
            <div>
              ...
              <input type="file" id="evidenceFiles" multiple accept=".zip,.csv,.pdf,.docx,.xlsx,.xlsm,.xer,.xml">
...
function fileType(file){const n=file.name.toLowerCase();if(n.endsWith(".zip"))return"application/zip";if(n.endsWith(".xer"))return"text/plain";if(n.endsWith(".xml"))return"application/xml";if(n.endsWith(".csv"))return"text/csv";if(n.endsWith(".pdf"))return"application/pdf";if(n.endsWith(".docx"))return"application/vnd.openxmlformats-officedocument.wordprocessingml.document";if(n.endsWith(".xlsm"))return"application/vnd.ms-excel.sheet.macroEnabled.12";return file.type||"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"}
...
function renderModuleResult(result){el("moduleTitle").textContent=names[result.key]||result.key;el("moduleBadge").className="badge "+statusClass(result.status);el("moduleBadge").textContent=result.status;if(result.status==="blocked"){el("moduleContent").innerHTML='<div class="notice warn"><b>Blocked by evidence dependency</b><br>'+escapeHtml(result.reason||"Required evidence is not established.")+'</div><div class="scalar-grid">'+(result.dependencies||[]).map(x=>'<div class="scalar"><b>Required</b><span>'+escapeHtml(x)+'</span></div>').join("")+'</div>';return}const data=result.data||{};const scalars=scalarPairs(data).map(([k,v])=>'<div class="scalar"><b>'+escapeHtml(k)+'</b><span>'+escapeHtml(fmt(v))+'</span></div>').join("");el("moduleContent").innerHTML=(result.reason?'<div class="notice info">'+escapeHtml(result.reason)+'</div>':'')+'<div class="scalar-grid">'+scalars+'</div><details><summary>Evidence-backed module output</summary><pre>'+escapeHtml(JSON.stringify(data,null,2))+'</pre></details>'}
