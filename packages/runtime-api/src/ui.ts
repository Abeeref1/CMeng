...
function renderRoleLens(key,data,role){
  ...
  if(role==="overall"){
    const layers=[
      ["Position","Governed current position"],
      ["Exceptions","Gaps, variance and exposure"],
      ["Decision path","Action and evidence"]
    ];
    return '<section class="role-lens role-lens-compact role-lens-summary"><div class="role-lens-compact-strip">'+layers.map(([title,text])=>'<div><b>'+escapeHtml(title)+'</b><span>'+escapeHtml(text)+'</span></div>').join("")+'</div></section>';
  }
...
function renderEotVisual(data){
  ...
  const movementVisual=renderWaterfallChart([
    {label:"Gross positive window movement",value:p.observedProgrammeMovementDays},
    {label:"Net Project Completion movement",value:p.projectCompletionMovementDays},
    {label:"Analytical time-impact candidate",value:analytical},
    {label:"Attributable EOT candidate",value:p.attributableCandidateEotDays}
  ].filter(row=>typeof row.value==="number"),"d");
  const visualOverview='<div class="visual-chart-grid">'+
...
function renderCommercialVisual(key,data){
  ...
  if(contractControls){
    ...
    if(key==="contract-particulars-bonds"){
      ...
      const obligationVisual=renderVisualPanel(
        ...
        obl.state==="missing"
          ?'<div class="empty-visual">Obligation register is not established. Zero is not inferred.</div>'
          :renderVisualBars([
      ...
      const securityVisual=renderVisualPanel(
        ...
        "Active, expiring and expired counts are shown only when a governed register exists.",
        bi.state==="missing"
          ?'<div class="empty-visual">Bond/security and insurance registers are not established. Zero instruments are not inferred.</div>'
          :renderVisualBars([
      ...
      const retentionVisual=renderVisualPanel(
        ...
        "Deduction records, held-state records, current held balance, release dates and overdue status remain distinct.",
        renderVisualBars([
          {label:"Held-state records",value:ret.heldCount??0,tone:"warning"},
      ...
      contractControlDetail=
        ...
        planningKpis([
          ...
          ["Retention deduction records",ret.recordCount||0,"deduction/calendar source rows"],
          ["Held-state records",ret.heldCount||0,"record state; current held balance is separately reconciled"],
          ["Retention overdue",ret.dueCount?ret.overdueCount||0:"Not assessable","requires established release due date"]
        ])+
        ...
        planningKpis([
          ...
          ["Deduction / retention records",ret.recordCount||0,"all evidence origins"],
          ["Held-state records",ret.heldCount||0,"record state; not a reconciled held balance"],
          ["Released",ret.releasedCount||0,"explicit release state"],
          ["Dated",ret.dueCount||0,"release date/trigger established"],
          ["Overdue",ret.dueCount?ret.overdueCount||0:"Not assessable","past due requires an established release date"]
        ])+
  ...
  if(key==="commercial-claims-notices"){
    ...
    detail='<section class="planning-panel primary commercial-claims-management"><div class="planning-panel-head"><div><h4>Commercial Claims & Notices Management Position</h4><p>Claim lifecycle, contractual notice compliance and financial exposure reuse governed identities. Missing amounts or links remain unknown rather than zero.</p></div></div><div class="planning-panel-body">'+
      ...
      table(["Claim","Title","State","Submitted","Claimed days","Assessed days","Assessment authority","Events","Clauses","Evidence refs"],lifecycleRows,"No governed claim lifecycle is established.")+
      ...
      table(["Notice","Type","Event","Claim","Issued","Received","Subject","Clauses","Evidence refs"],noticeRows,"No governed notice register is established.")+
