...
function renderResourceVisual(data){
  ...
  const plannedUtil=typeof weekly?.plannedAverageToDataDate==="number"?weekly.plannedAverageToDataDate:null;
  const actualUtil=typeof weekly?.actualAverageToDataDate==="number"?weekly.actualAverageToDataDate:null;

  const kpis=planningKpis([
    ["Resources",p.resourceCount,"resource master"],
    ["Assigned",p.assignedResourceCount,"with programme assignments"],
    ["Capacity coverage",weekly?.capacityCoveragePercent==null?"Not established":fmt(weekly.capacityCoveragePercent)+"%","resource-week evidence",weekly?.capacityCoveragePercent==null?"warning":"accent"],
    ["Planned utilization",plannedUtil===null?"Not established":fmt(plannedUtil)+"%","to Data Date",plannedUtil===null?"warning":"accent"],
    ["Actual utilization",actualUtil===null?"Not established":fmt(actualUtil)+"%","approved usage to Data Date",actualUtil===null?"warning":"success"],
    ["Demand > capacity",weeklyComparable?weeklyOver:"Not assessable",weeklyComparable?fmt(weeklyComparable)+" comparable weekly checks":"capacity evidence required",weeklyOver?"danger":weeklyComparable?"success":"warning"]
  ]);

  const utilizationVisual=renderVisualPanel(
    "Utilization position",
    "Planned and approved actual utilization use the governed weekly resource-capacity source and remain in percentage terms.",
    renderVisualBars([
      {label:"Planned utilization",value:plannedUtil,tone:"accent"},
      {label:"Actual utilization",value:actualUtil,tone:"success"}
    ].filter(item=>item.value!==null),"%"),
    weekly?.capacityCoveragePercent==null?"Coverage not established":fmt(weekly.capacityCoveragePercent)+"% capacity coverage"
  );

  const weeklyCharts=weeklyGroups.size
    ? [...weeklyGroups.entries()].map(([unit,points])=>renderVisualPanel(
        "Capacity vs demand · "+unit,
        "Available capacity, planned demand and approved usage stay in their original source unit. Different units are never added.",
        renderLineChart(points,[
          {key:"availableCapacity",label:"Available capacity",tone:"graphite"},
          {key:"plannedDemand",label:"Planned demand",tone:"warning"},
          {key:"actualApprovedUsage",label:"Approved actual usage",tone:"success"}
        ]),
        fmt(points.length)+" weeks"
      )).join("")
    : renderVisualPanel(
        "Capacity vs demand",
        "A time-phased capacity chart is withheld until comparable weekly capacity and demand evidence exists.",
        '<div class="empty-visual">No comparable weekly capacity series is established.</div>'
      );
  ...
  const concentrationVisual=renderVisualPanel(
    "Per-hour demand concentration",
    "Top established planned/remaining demand rates. Assignment count is never used as a demand substitute.",
    moduleBarList(comparableDemand)
  );

  const evidenceMessages=[];
  if(capacityKnown===0){
    evidenceMessages.push('<div class="notice warn"><b>Per-hour overload is not zero; it is not assessable.</b> Assigned resources do not have a usable max-units-per-hour capacity in the programme source.</div>');
  }else if(p.capacityCoveragePercent<100){
    evidenceMessages.push('<div class="notice info">Per-hour utilization is calculated only for resources with established comparable capacity. The remainder stays unassessed.</div>');
  }
  if(weeklyComparable){
    evidenceMessages.push('<div class="notice info"><b>Weekly source capacity is established.</b> '+escapeHtml(fmt(weeklyOver))+' of '+escapeHtml(fmt(weeklyComparable))+' comparable resource-week checks exceed capacity. Unit classes remain isolated.</div>');
  }

  const weeklyDetailRows=(weekly?.resourceSummaries||[]).map(r=>'<tr><td><b>'+escapeHtml(r.resourceId)+'</b><br>'+escapeHtml(r.resourceName||"")+'</td><td>'+escapeHtml(r.resourceClass)+'</td><td>'+escapeHtml(r.unit||"Not established")+'</td><td>'+escapeHtml(fmt(r.periodCount))+'</td><td>'+escapeHtml(r.plannedUtilizationPercent===null?"Not established":fmt(r.plannedUtilizationPercent)+"%")+'</td><td>'+escapeHtml(r.actualUtilizationPercent===null?"Not established":fmt(r.actualUtilizationPercent)+"%")+'</td></tr>').join("");
  const resourceRows=p.rows.map(r=>'<tr><td><b>'+escapeHtml(r.resourceId)+'</b><br><span class="muted">'+escapeHtml(r.resourceName||"")+'</span></td><td>'+escapeHtml(r.resourceType)+'</td><td>'+escapeHtml(r.assignmentCount)+'</td><td>'+escapeHtml(fmt(r.capacityUnitsPerHour))+'</td><td>'+escapeHtml(fmt(r.peakPlannedUnitsPerHour))+'</td><td>'+escapeHtml(fmt(r.peakRemainingUnitsPerHour))+'</td><td>'+escapeHtml(r.plannedUtilizationPercent===null?"—":fmt(r.plannedUtilizationPercent)+"%")+'</td><td>'+escapeHtml(r.remainingUtilizationPercent===null?"—":fmt(r.remainingUtilizationPercent)+"%")+'</td><td><span class="state-pill '+(r.overloaded===true?"blocked":r.state==="capacity_based"?"ready":"review")+'">'+escapeHtml(r.overloaded===true?"Overloaded":r.state==="capacity_based"?"Capacity assessed":"Capacity not set")+'</span></td></tr>').join("");

  const evidenceDetail='<details class="role-supporting-detail resource-supporting-detail"><summary><b>Capacity evidence & source coverage</b><span>Open evidence basis</span></summary><div class="role-supporting-detail-body">'+
    evidenceMessages.join("")+
    moduleEvidenceGate([
      ...
      {label:"Capacity units",value:weeklyUnits.length?weeklyUnits.join(" / "):"Not established",state:weeklyUnits.length?"ready":"missing"},
      {label:"Per-hour overload assessment",value:assessed>0?fmt(p.perHourOverloadedResourceCount??p.overloadedResourceCount):"Not assessable",state:assessed>0?"ready":"missing"}
    ])+
    (weeklyDetailRows?'<div class="section-heading compact"><div><h5>Weekly utilization by resource</h5><p>Means of comparable resource-week ratios to the programme Data Date.</p></div></div><div class="table-wrap"><table><thead><tr><th>Resource</th><th>Class</th><th>Unit</th><th>Periods</th><th>Planned util.</th><th>Actual util.</th></tr></thead><tbody>'+weeklyDetailRows+'</tbody></table></div>':'')+
    '</div></details>';

  const resourceDetail='<details class="role-supporting-detail resource-supporting-detail"><summary><b>Full resource detail</b><span>'+escapeHtml(fmt(p.rows.length))+' resources</span></summary><div class="role-supporting-detail-body"><div class="table-wrap"><table><thead><tr><th>Resource</th><th>Type</th><th>Assignments</th><th>Capacity/hr</th><th>Peak planned/hr</th><th>Peak remaining/hr</th><th>Planned util.</th><th>Remaining util.</th><th>Assessment</th></tr></thead><tbody>'+resourceRows+'</tbody></table></div></div></details>';

  return '<section class="planning-view resource-view resource-enterprise">'+
    '<section class="planning-panel primary resource-management-position"><div class="planning-panel-head"><div><h4>Resource Capacity & Utilization Position</h4><p>Capacity coverage, utilization and overload are shown from their governed source basis without mixing labor, equipment, material or units.</p></div></div><div class="planning-panel-body">'+kpis+'</div></section>'+
    '<div class="visual-chart-grid resource-primary-visuals">'+utilizationVisual+weeklyCharts+'</div>'+
    concentrationVisual+
    evidenceDetail+
    resourceDetail+
    '</section>';
...
function renderProgressReportVisual(data){
  ...
  const physicalVariance=physical?.varianceToBaselinePercentagePoints??null;
  ...
  const kpis=planningKpis([
    ["Baseline planned",baseline?.valuePercent===null?"Not established":fmt(baseline?.valuePercent)+"%","planned by Data Date"],
    ["Current programme plan",current?.valuePercent===null?"Not established":fmt(current?.valuePercent)+"%","re-phased expectation","accent"],
    [physicalLabel,physical?.valuePercent===null?"Not established":fmt(physical?.valuePercent)+"%",physical?.authority==="source_evidence"?"source physical evidence":"programme progress snapshot",physical?.valuePercent===null?"warning":"accent"],
    ["Vs baseline",physicalVariance===null?"Not established":(physicalVariance>0?"+":"")+fmt(physicalVariance)+" pp","producer-calculated variance",physicalVariance!==null&&physicalVariance<0?"danger":physicalVariance!==null&&physicalVariance>0?"success":""],
    ["Contractor reported",contractor?.valuePercent===null?"Not provided":fmt(contractor.valuePercent)+"%","source evidence only",contractor?.valuePercent===null?"warning":"accent"],
    ["Certified progress",certified?.valuePercent===null?"Not provided":fmt(certified.valuePercent)+"%","source evidence only",certified?.valuePercent===null?"warning":"success"]
  ]);

  const warning=!sourceProgressEstablished
    ? '<div class="notice warn"><b>Certified/contractor physical progress is not established.</b> The programme percentage-complete snapshot remains visible but is not relabelled as certified progress.</div>'
    : '';
  ...
  const executionDonut=renderDonutChart([
    {label:"Completed",value:p.progress?.completedCount||0,tone:"success"},
    {label:"In progress",value:p.progress?.inProgressCount||0,tone:"accent"},
  ...
  const pressureCards=planningKpis([
    ["Critical",p.schedule?.criticalCount??0,"TF at/below critical threshold",p.schedule?.criticalCount?"danger":""],
    ["Near-critical",p.schedule?.nearCriticalCount??0,"strict near-critical population",p.schedule?.nearCriticalCount?"warning":""],
    ["Negative float",p.schedule?.negativeFloatCount??0,"schedule pressure",p.schedule?.negativeFloatCount?"danger":""],
    ["Float coverage",p.schedule?.floatCoveragePercent===null?"Not established":fmt(p.schedule.floatCoveragePercent)+"%","activities with usable float",p.schedule?.floatCoveragePercent===null?"warning":"accent"]
  ]);

  const basisRows=Object.entries(p.progressBases).map(([key,basis])=>{
    const variance=basis?.varianceToBaselinePercentagePoints;
    const authority=basis?.authority==="progress_snapshot"?"Schedule snapshot":basis?.authority==="deterministic_schedule"?"Schedule calculation":basis?.authority==="source_evidence"?"Source evidence":"Not provided";
    return '<tr><td><b>'+escapeHtml(progressBasisDisplay(key,basis))+'</b></td><td>'+escapeHtml(basis?.valuePercent===null?"Not established":fmt(basis.valuePercent)+"%")+'</td><td>'+escapeHtml(authority)+'</td><td>'+escapeHtml(planningShortDate(basis?.asOfIso))+'</td><td>'+escapeHtml(basis?.coveragePercent===null||basis?.coveragePercent===undefined?"Not established":fmt(basis.coveragePercent)+"%")+'</td><td>'+escapeHtml(variance===null||variance===undefined?"—":(variance>0?"+":"")+fmt(variance)+" pp")+'</td></tr>';
  }).join("");
  const basisEvidence='<details class="role-supporting-detail progress-basis-detail"><summary><b>Progress evidence & authority</b><span>Open five governed progress bases</span></summary><div class="role-supporting-detail-body"><div class="table-wrap"><table><thead><tr><th>Basis</th><th>Value</th><th>Authority</th><th>As of</th><th>Coverage</th><th>Vs baseline</th></tr></thead><tbody>'+basisRows+'</tbody></table></div></div></details>';

  return '<section class="planning-view progress-position-view progress-enterprise">'+
    '<section class="planning-panel primary progress-management-position"><div class="planning-panel-head"><div><h4>Progress Management Position</h4><p>Baseline plan, current plan, programme snapshot, contractor-reported and certified progress remain separate and authority-labelled.</p></div></div><div class="planning-panel-body">'+kpis+warning+'</div></section>'+
    '<div class="visual-chart-grid progress-primary-visuals">'+
      renderVisualPanel("Progress basis comparison","Comparable percentages only. Schedule progress is not presented as certified physical progress.",progressChart)+
      renderVisualPanel("Execution status","Current programme activity population by execution state.",executionDonut)+
    '</div>'+
    '<section class="planning-panel"><div class="planning-panel-head"><div><h4>Schedule pressure</h4><p>Criticality and float pressure are schedule indicators, not progress evidence.</p></div></div><div class="planning-panel-body">'+pressureCards+'</div></section>'+
    '<section class="planning-panel"><div class="planning-panel-head"><div><h4>Near-term delivery</h4><p>Milestone and look-ahead pressure tied to the current programme Data Date.</p></div></div><div class="planning-panel-body">'+planningKpis([
      ...
      ["Date coverage",p.lookAhead?.currentDateCoveragePercent===null?"Not established":fmt(p.lookAhead?.currentDateCoveragePercent)+"%","look-ahead dates",p.lookAhead?.currentDateCoveragePercent===null?"warning":"accent"]
    ])+'</div></section>'+
    basisEvidence+
    '</section>';
}
...
function renderManagementCommercial(rows){
  ...
  return '<div class="table-wrap"><table><thead><tr><th>Currency</th><th>Pending variations</th><th>Approved variations</th><th>Certified unpaid</th><th>Retention held</th><th>Active bonds</th><th>Claimed</th><th>Assessed</th><th>LD scenario</th></tr></thead><tbody>'+rows.map(r=>'<tr><td><b>'+escapeHtml(r.currency)+'</b></td><td>'+escapeHtml(fmt(r.pendingVariationAmount))+'</td><td>'+escapeHtml(fmt(r.approvedVariationAmount))+'</td><td>'+escapeHtml(fmt(r.certifiedUnpaidAmount))+'</td><td>'+escapeHtml(fmt(r.retentionHeldAmount))+'</td><td>'+escapeHtml(fmt(r.activeBondAmount))+'</td><td>'+escapeHtml(fmt(r.claimClaimedAmount))+'</td><td>'+escapeHtml(fmt(r.claimAssessedAmount))+'</td><td>'+escapeHtml(fmt(r.ldScenarioAmount))+'</td></tr>').join("")+'</tbody></table></div>';
}
