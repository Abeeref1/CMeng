...
"cash-flow":"Actual cash availability, certificate reconciliation and the forward source profile.",
...
const fmt=v=>v===null||v===undefined?"—":typeof v==="number"?new Intl.NumberFormat(undefined,{maximumFractionDigits:2}).format(v):humanizeIsoText(String(v));
...
function renderDeliveryChallenge(data,reason,status){
  ...
  const gates=moduleEvidenceGate([
    {label:"Parsed contract clauses",value:contract?fmt(contract.clauseCount||0)+" parsed clauses":"Not established",state:contract?.semanticComplete===true&&contract?.signalCount>0?"ready":"partial"},
    {label:"Explicit headcount plan",value:submittedManpower?"Established":"Headcount not established",state:submittedManpower?"ready":"missing"},
  ...
  const scheduleCards=forecastReview+planningKpis([
    ...
    ["Average concurrent activities",s.averageConcurrentWorkFronts,"task concurrency; work fronts not established"],
    ["Peak concurrent activities",s.peakConcurrentWorkFronts,"task concurrency; work fronts not established"]
  ]);
  ...
  '</div>';
  const contractSources=contract?.sourceDocuments?.length?'<div class="table-wrap"><table><thead><tr><th>Source document</th><th>Role / authority</th><th>Clauses</th><th>Topic occurrences</th><th>Notice candidates</th></tr></thead><tbody>'+contract.sourceDocuments.map(x=>'<tr><td>'+escapeHtml(x.sourceFilename)+'</td><td>'+escapeHtml(humanizeKey(x.role))+' / '+escapeHtml(humanizeKey(x.basisState))+'</td><td>'+escapeHtml(fmt(x.clauseCount))+'</td><td>'+escapeHtml(fmt(x.signalCount))+'</td><td>'+escapeHtml(fmt(x.noticeCandidateCount))+'</td></tr>').join('')+'</tbody></table></div>':'';
...
function renderCommercialValueBridge(base,approved,current,pending,unit=""){
  ...
  return '<div class="value-bridge">'+
    cell(sourceSnapshot?"Source original contract":"Committed contract",base,"base")+
...
function renderLineChart(points,series,yMaxHint=null,options={}){
  if(points.length===1){const point=points[0];return '<div class="notice info"><b>Single observation · trend not established</b><p>'+escapeHtml(planningShortDate(point.dateIso||point.asOf))+'</p>'+series.filter(item=>typeof point[item.key]==='number').map(item=>'<div class="currency-line"><span>'+escapeHtml(item.label)+'</span><strong>'+escapeHtml(fmt(point[item.key])+(options.unit?' '+options.unit:''))+'</strong></div>').join('')+'</div>';}
...
function renderForecastVisual(data){
  ...
  const kpis=planningKpis([
    ...
    ["Source Productivity Forecast",planningShortDate(p.sourceProductivityForecastCompletionIso),p.sourceProductivityForecastCompletionIso?"source evidence / derived forecast":"Not established",p.sourceProductivityForecastState==="conflicted"?"danger":p.sourceProductivityForecastState==="candidate"?"warning":""],
  ...
  const forecastDistance=[
    ...
    {label:"Calendar recalculation",value:planningCalendarDaysBetween(p.dataDateIso,p.independentForecastCompletionIso),tone:"accent"},
  ...
  const dateLadder=planningDateLadder([
    ...
    {label:"Calendar recalculation",date:p.independentForecastCompletionIso,tone:"cmeng"},
...
function renderWindowsVisual(data){
  ...
  return '<section class="planning-view windows-view">'+planningKpis([
    ...
    ["Analytical vs net difference",p.analyticalVsNetDeltaDays==null?"Not established":fmt(p.analyticalVsNetDeltaDays)+" d","difference in bases; not proven overlap"],["Partial windows",p.partialWindowCount,"",p.partialWindowCount?"warning":""],
...
function renderDelayClaimsVisual(data){
  ...
  const kpis=planningKpis([
    ...
    ["Gross positive window movement",movementEstablished?fmt(p.observedPositiveProgrammeMovementDays)+" d":"Not established","submitted programme window context; not event attribution",movementEstablished&&p.observedPositiveProgrammeMovementDays?"warning":""],
  ...
  const rows=events.map(e=>'<tr><td><b>'+escapeHtml(e.eventId)+'</b><br><span class="muted">'+escapeHtml(e.title||"")+'</span></td><td>'+escapeHtml(humanizeKey(e.responsibility))+'</td><td>'+escapeHtml(humanizeKey(e.noticeTimeliness))+'</td><td>'+escapeHtml(e.observedPositiveProgrammeMovementDays==null?'Not established':fmt(e.observedPositiveProgrammeMovementDays))+'<br><small>Window context only</small>'+'</td><td>'+escapeHtml(humanizeKey(e.programmeMovementBasis))+'</td><td>Not causally attributed</td><td><span class="state-pill '+(e.evidenceChainMissingLinks?.length||e.concurrencyCandidate?"review":"ready")+'">'+escapeHtml(humanizeKey(e.evidenceChainState||e.candidateClass))+'</span></td><td>'+escapeHtml((e.linkedClaimIds||[]).join(", ")||"—")+'</td><td>'+escapeHtml((e.relatedActivityIds||[]).join(", ")||"—")+'</td><td>'+escapeHtml((e.overlappingWindowIds||[]).map(id=>readableWindow(id,p.revisionLabels||{})).join(", ")||"—")+'</td><td>'+escapeHtml((e.noticeIds||[]).join(", ")||"—")+'</td><td>'+escapeHtml((e.determinationIds||[]).join(", ")||"—")+'</td><td>'+escapeHtml((e.evidenceChainMissingLinks||[]).map(humanizeKey).join(", ")||"—")+'</td></tr>').join("");
  const detail=events.length?'<div class="table-wrap"><table><thead><tr><th>Event</th><th>Responsibility</th><th>Notice status</th><th>Window context d · non-additive</th><th>Movement basis</th><th>Schedule attribution</th><th>Evidence chain</th><th>Claims</th><th>Activities</th><th>Windows</th><th>Notices</th><th>Determinations</th><th>Missing links</th></tr></thead><tbody>'+rows+'</tbody></table></div>':'<div class="empty-visual">No delay-event population is established. Claim records alone are not converted into delay events.</div>';
...
function renderEotVisual(data){
  ...
  const kpis=planningKpis([
    ...
    ["Submitted finish vs contract",planningCalendarDaysBetween(p.contractualCompletionIso,p.sourceForecastCompletionIso)===null?"Not established":fmt(planningCalendarDaysBetween(p.contractualCompletionIso,p.sourceForecastCompletionIso))+" calendar days","submitted programme minus current amended contract","warning"],
    ["Gross determinations by Data Date",p.officialApprovedEotDays===null?"Not established":fmt(p.officialApprovedEotDays)+" d","may already be incorporated in the amendment"],
    ["Further adjusted contractual completion",p.officialAdjustedCompletionIso?planningShortDate(p.officialAdjustedCompletionIso):"Not established","governed only"],
    ["Gross positive window movement",movementEstablished?fmt(p.observedProgrammeMovementDays)+" d":"Not established","submitted programme window context; not EOT",movementEstablished&&p.observedProgrammeMovementDays>0?"warning":""],
  ...
  const movementBars=p.windowCandidates.map((w,index)=>({
    ...
    value:typeof w.positiveProgrammeMovementDays==="number"?w.positiveProgrammeMovementDays:null,
    tone:"warning"
  }));
  ...
  return '<section class="planning-view eot-view">'+kpis+reconciliation+warning+visualOverview+'<div class="planning-primary-grid"><section class="planning-panel primary"><div class="planning-panel-head"><div><h4>Independent CPM movement by window</h4><p>Analytical revision-to-revision CPM movement; not submitted movement or EOT entitlement.</p></div></div><div class="planning-panel-body">'+planningSignedBars(movementBars,"days")+'</div></section><section class="planning-panel"><div class="planning-panel-head"><div><h4>EOT evidence gates</h4><p>All gates remain distinct before schedule movement can become an entitlement position.</p></div></div><div class="planning-panel-body">'+moduleEvidenceGate([
    ...
    {label:"Causally attributable EOT-eligible events",value:causalReady?"Established":"Not established",state:causalReady?"ready":"missing"},
    {label:"Gross determined days by Data Date",value:p.officialApprovedEotState==="official"?"Established":"Not established",state:p.officialApprovedEotState==="official"?"ready":"missing"}
...
function planningStateLabel(value){
  const labels={
    ...
    true:"Yes",false:"No",established:"Established",not_established:"Not established",
...
function planningMilestoneTimeline(p){
  ...
  return '<div class="milestone-date-legend"><span>○ Controlled baseline</span><span><b>◆ Current forecast</b></span><span>Open milestones only · Data Date '+escapeHtml(planningShortDate(p.dataDateIso))+'</span></div><div class="milestone-date-chart">'+rows.map(r=>{
    ...
    return '<div class="milestone-date-row"><div class="milestone-date-name"><b>'+escapeHtml(r.activityId)+' · '+escapeHtml(r.name||'')+'</b><span>'+escapeHtml(planningMilestoneCriticalityLabel(r))+' · '+escapeHtml(planningMilestoneDueLabel(r))+'</span><small>'+escapeHtml(r.wbsName||r.wbsId||'')+'</small>'+(r.managementPriority==='watch'&&count>1?'<small>Representative of '+escapeHtml(fmt(count))+' watch milestones with the same movement. This does not establish a common cause.</small>':'')+'</div><div>'+axis+'<div class="milestone-date-track">'+track+'</div><div class="milestone-date-values"><span>Baseline <b>'+escapeHtml(planningShortDate(r.baselineDateIso))+'</b></span><span>Current <b>'+escapeHtml(planningShortDate(r.currentDateIso))+'</b></span><span>Movement <b>'+escapeHtml(variance)+'</b></span><span>Float <b>'+escapeHtml(r.totalFloatHours==null?'Not established':fmt(r.totalFloatHours)+' h')+'</b></span></div></div></div>';
...
function renderPmoVisual(data){
  ...
  const completion=planningDateLadder([
    ...
    {label:"Source productivity forecast",date:p.sourceProductivityForecast?.completionIso,tone:"current"},
  ...
  const health='<div class="management-health-grid">'+[
    ...
    ["Delivery",[
      ["Assigned resources",p.resources.assignedResourceCount],["Weekly capacity evidence coverage",p.resources.weeklyCapacityCoveragePercent===null?"—":fmt(p.resources.weeklyCapacityCoveragePercent)+"%"],["Actual overloads through DD",p.resources.capacityChecksToDataDate?fmt(p.resources.capacityChecksToDataDate.actual.exceededCount)+" / "+fmt(p.resources.capacityChecksToDataDate.actual.comparableCount)+" resource-weeks":"Not established"],["BOQ/activity link",planningStateLabel(p.quantities.allocationState)]
    ]],
    ["Claims & time",[
...
function renderScheduleAnalyticsVisual(data){
  ...
  const kpis=planningKpis([
    ...
    ["Source open starts",openStarts,"full source graph","warning"],
    ["Source open finishes",openFinishes,"full source graph","warning"],
  ...
  const integrity='<div class="integrity-grid">'+[
    ["Cycles",cycles,cycles?"danger":"success"],["Broken links",broken,broken?"danger":"success"],["Source open starts",openStarts,openStarts?"warning":"success"],["Source open finishes",openFinishes,openFinishes?"warning":"success"],["Source isolated activities",isolated,isolated?"warning":"success"],["Graph computational integrity",r.graph.complete?"Complete":"Review needed",r.graph.complete?"success":"danger"]
...
const movementClusters=new Map();
  ...
  const authority=planningKpis([
    ["Threshold authority",humanizeKey(p.controlBasis?.state||"missing"),humanizeKey(p.controlBasis?.nearCriticalThresholdMethod||"unresolved"),p.controlBasis?.state==='official'?'':'warning'],
    ["Governed contract completion",planningShortDate(p.contractualCompletionIso),"contract evidence, separate from activity names"],
...
function renderForecastHistoryVisual(data){
  ...
  const cards=p.points.map((x,index)=>'<div class="revision-value-card"><div class="revision-value-head"><b>'+escapeHtml(shortRevision(x.sourceRevisionId,labels))+'</b><span>'+escapeHtml(planningShortDate(x.dataDateIso))+'</span></div><div class="revision-value-lines"><span>Submitted finish <b>'+escapeHtml(planningShortDate(x.sourceForecastCompletionIso))+'</b></span><span>Independent finish <b>'+escapeHtml(planningShortDate(x.independentForecastCompletionIso))+'</b></span><span>Submitted move vs prior <b>'+escapeHtml(index===0?'N/A':fmt(planningCalendarDaysBetween(p.points[index-1].sourceForecastCompletionIso,x.sourceForecastCompletionIso))+' d')+'</b></span><span>Submitted move vs first <b>'+escapeHtml(index===0?'N/A':fmt(planningCalendarDaysBetween(p.points[0].sourceForecastCompletionIso,x.sourceForecastCompletionIso))+' d')+'</b></span><span>Independent move vs prior <b>'+escapeHtml(x.movementDaysVsPrevious===null?"—":fmt(x.movementDaysVsPrevious)+" d")+'</b></span></div></div>').join("");
...
function renderNoticesClaimsVisual(data){
  ...
  const kpis=planningKpis([
    ...
    ["Determined days by Data Date",p.effectiveDeterminationDays==null?"Not established":fmt(p.effectiveDeterminationDays)+" d","gross register days; amendment overlap unresolved","warning"],
    ["Provisional / candidate days",p.provisionalOrCandidateAssessedDaysTotal==null?"Not established":fmt(p.provisionalOrCandidateAssessedDaysTotal)+" d","claim-row statistic; non-additive; not project EOT"],
    ...
    ["Requirements missing",p.noticeRequirementMissingCount??"Not established","known evidence gaps; not a compliance outcome"]
  ]);
  const contractNotice='<div class="notice info"><b>Contract notice period: '+escapeHtml(p.contractNoticePeriod?.value==null?'Not established':fmt(p.contractNoticePeriod.value)+' days')+'</b>. A period alone does not establish the trigger, day basis or applicability to each event. These dimensions require explicit mapping before timeliness can be assessed.</div>';
  const warning=contractNotice+(!assessable?'<div class="notice warn"><b>Notice performance is not zero; it is not assessable.</b> A claim row or notice date by itself does not prove notice compliance. CMeng needs a governed delay event and the applicable contractual notice requirement before classifying notice as timely, late or missing.</div>':partiallyAssessable?'<div class="notice warn"><b>Notice compliance is only partially assessable.</b> Established event/requirement pairs are assessed below, while events without an applicable governed notice requirement remain explicit evidence gaps.</div>':'');
...
function renderCommercialLedgerVisual(data,scoped=false){
  ...
  if(!scoped&&["commercial-payment-register","commercial-variations","commercial-cost-register"].includes(key)){
    ...
    return renderCommercialLedgerVisual({...p,rows:current,scope:"as_of",summary:{...p.summary,effectiveRecordCount:current.length}},true)+group("After Data Date, excluded from current totals",future,"future")+group("Date not established, excluded from current totals",undated,"undated");
  }
...
function renderCommercialVisual(key,data){
  ...
  const time=planningKpis([
    ...
    ["Gross determined days",t.approvedEotDays?.value===null||t.approvedEotDays?.value===undefined?"Not established":fmt(t.approvedEotDays.value)+" d",humanizeKey(t.approvedEotDays?.state||"not_submitted")],
    ["Further adjusted contractual completion",t.officialAdjustedCompletion?.value?planningShortDate(t.officialAdjustedCompletion.value):"Not established",humanizeKey(t.officialAdjustedCompletion?.state||"not_submitted")],
  ...
  const colsByKey={
    "commercial-overview":[["Committed","committedContractValue"],["Current contract","currentContractValue"],["Pending variations","pendingVariationAmount"],["Certified","grossCertifiedAmount"],["Paid","paidAmount"],["Retention deductions evidenced through DD","retentionDeductedAmount"],["Held balance","retentionHeldAmount"],["Claims","claimedAmount"],["Active bonds","activeBondAmount"]],
    ...
    "payments":[["Certificates","interimCertificateCount"],["Gross certified","grossCertifiedAmount"],["Paid","paidAmount"],["Certified unpaid","certifiedUnpaidAmount"],["Retention deductions evidenced through DD","retentionDeductedAmount"],["Held balance","retentionHeldAmount"],["Advance balance","advanceBalance"]],
    "cash-flow":[["Gross certified","grossCertifiedAmount"],["Paid","paidAmount"],["Certified unpaid","certifiedUnpaidAmount"],["Retention deductions evidenced through DD","retentionDeductedAmount"],["Held balance","retentionHeldAmount"],["Advance balance","advanceBalance"]],
  ...
  if(foundation){
    ...
    if(key==="payments"||key==="cash-flow"){
      ...
      const lifecycleVisual=renderVisualPanel(
        ...
        ],"records"):'<div class="empty-visual">Payment register is not established. Lifecycle counts are not established.</div>'
      );
      ...
      const slaVisual=renderVisualPanel(
        ...
        slaAssessable
          ...
          :paymentPopulationEstablished?'<div class="notice warning"><b>Not assessable</b><p>'+escapeHtml(fmt(slaCounts.notEstablished??0))+' payment record(s) do not have sufficient governed SLA event dates.</p></div>':'<div class="empty-visual">Payment register is not established. SLA counts are not established.</div>'
      );
  ...
  if(performance){
    ...
    if(key==="cost-forecast"){
      const controlSections=(performance.costControl?.positions||[]).map(row=>{
        const scenarioRows=(row.eacScenarios||[]).map(s=>'<tr><td><b>'+escapeHtml(humanizeKey(s.method))+'</b></td><td>'+escapeHtml(findingValue(s.value,row.currency))+'</td><td>'+escapeHtml(s.method==="source_reported"?(s.official?"Source / governed":"Source / requires review"):"Scenario only")+'</td><td>'+escapeHtml(s.methodology||"")+'</td></tr>');
        const varianceRows=[
      ...
      const cashSections=(performance.cashFlow?.currencies||[]).map(row=>{
        ...
        const curve=hasCurve
          ? '<div class="cash-flow-primary">'+renderVisualPanel(
              ...
              'Cumulative observed receipts and expenditure remain separate. Opening liquidity, facilities and future receipts are not established by this history.',
              renderLineChart(points,[
  ...
  if(contractControls){
    ...
    if(key==="variations-change"){
      ...
      const lifecycleVisual=renderVisualPanel(
        ...
        ],"items"):'<div class="empty-visual">Variation register is not established. Lifecycle counts are not established.</div>'
      );
      const agingVisual=renderVisualPanel(
        ...
        ],"items"):'<div class="empty-visual">Variation aging is not assessable until a governed variation population is established.</div>'
      );
      const bridgeVisuals=(position.currencies||[]).map(row=>renderVisualPanel(
        row.currency+" · Contract value & change bridge",
        "Source-reported contract values retain their own authority. Reconciliation against dated change records is required before adopting them as a governed contract-value position.",
      ...
      const instructionPressure=renderVisualPanel(
        ...
        ],"items"):'<div class="empty-visual">Site Instruction register is not established. Conversion and quotation counts are not established.</div>'
      );
    ...
    if(key==="contract-particulars-bonds"){
      ...
      const obligationVisual=renderVisualPanel(
        ...
        obligationsEstablished
          ? renderVisualBars([
              ...
              {label:"Promoted clause candidates",value:obl.clauseCandidateCount??0,tone:"warning"}
            ],"items")
      ...
      const retentionVisual=renderVisualPanel(
        ...
        (typeof ret.overdueCount==="number"?"":'<div class="notice info" style="margin-top:10px">Deduction rows do not establish held or released balances. Overdue retention is not assessable until release due dates or contractual release triggers are established.</div>'):'<div class="empty-visual">No retention register or governed balance records are established. Held, released and due counts are not established.</div>'
      );
      ...
      contractControlDetail=
        ...
        planningKpis([
          ["Controlled obligation records",obligationsEstablished?(obl.explicitRecordCount??0):"Not established","explicit register"],
          ["Promoted clause candidates",obl.clauseCandidateCount||0,"not yet mapped"],
  ...
  if(key==="commercial-claims-notices"){
    ...
    detail='<section class="planning-panel primary commercial-claims-management"><div class="planning-panel-head"><div><h4>Commercial Claims & Notices Management Position</h4><p>Claim lifecycle, contractual notice compliance and financial exposure reuse governed identities. Missing amounts or links remain unknown rather than zero.</p></div></div><div class="planning-panel-body">'+
      ...
      '<div class="notice info"><b>Contract notice period: '+escapeHtml(foundation?.commercialTerms?.noticePeriodDays?.value==null?'Not established':fmt(foundation.commercialTerms.noticePeriodDays.value)+' days')+'</b>. Period evidence is retained; trigger, day basis and event applicability remain unestablished until mapped. Evidence gaps are independent and can overlap. Source claim final stages and determination records have different identities and dates. Final source stages are not reconstructed historical stages at the Data Date.</div>'+
      ...
      '<div class="section-heading compact"><div><h5>Correspondence register</h5><p>Full source dates. Notices and determinations are separate types; future correspondence is excluded from the current decision basis.</p></div><span class="badge">'+escapeHtml(fmt((cn.notices||[]).length))+' correspondence rows</span></div>'+
  ...
  const temporalScope=renderCommercialTemporalPosition(ledger);
  const temporalWarning=ledgerIssues.length?'<div class="notice warn"><b>Commercial reconciliation required.</b> '+escapeHtml([...new Set(ledgerIssues)].map(humanizeKey).join('; '))+'. Source aggregate and dated line items retain separate authority until reconciled.</div>':'';
  if(ledger){
  ...
  const evidencePanel='<section class="planning-panel"><div class="planning-panel-head"><div><h4>Evidence coverage</h4><p>Source availability is separate from current lifecycle, reconciliation and analytical readiness shown above.</p></div></div><div class="planning-panel-body">'+gates+'</div></section>';
  if(key==="commercial-overview"){
  ...
  if(key==="payments"){
    return '<section class="planning-view commercial-view payments-enterprise">'+temporalWarning+temporalScope+
      foundationDetail+
...
function commercialFindingText(metric){
  ...
  return fmtExecutive(metric.value)+(metric.state&&metric.state!=="established"?" · "+humanizeKey(metric.state):"");
}
...
function renderManagementCommercial(rows){
  ...
  return '<div class="table-wrap"><table><thead><tr><th>Currency</th><th>Pending variations</th><th>Approved variations — source aggregate</th><th>Certified unpaid</th><th>Retention deductions evidenced through DD</th><th>Held balance</th><th>Active bonds</th><th>Claimed</th><th>Assessed</th><th>LD scenario</th></tr></thead><tbody>'+rows.map(r=>'<tr><td><b>'+escapeHtml(r.currency)+'</b></td>'+cell(r.pendingVariationAmount)+cell(r.approvedVariationAmount)+cell(r.certifiedUnpaidAmount)+cell(r.retentionDeductedAmount)+cell(r.retentionHeldAmount)+cell(r.activeBondAmount)+cell(r.claimClaimedAmount)+cell(r.claimAssessedAmount)+cell(r.ldScenarioAmount)+'</tr>').join("")+'</tbody></table></div>';
}
...
function renderManagementConsistency(c){
  ...
  return '<div class="notice '+(c.state==="pass"?"info":"warn")+'"><b>Cross-module consistency: '+escapeHtml(humanizeKey(c.state))+'</b><p>'+escapeHtml(c.scope||"")+'</p><span>'+escapeHtml(c.checkCount||0)+' checks'+(c.failedCheckIds?.length?' · Review: '+escapeHtml(c.failedCheckIds.join(", ")):"")+'</span></div>';
}
function renderManagementVariationReconciliation(rows){
  if(!rows.length)return "";
  return '<div class="notice info"><b>Variation basis reconciliation</b><p>The source aggregate and dated line-item approvals are separate authorities. Future approvals do not enter the as-of-DD amount. Reconciliation is required before an aggregate can be treated as the dated approved position.</p></div><div class="table-wrap"><table><thead><tr><th>Currency / tax basis</th><th>Source aggregate / state</th><th>Dated approvals through DD</th><th>Current / future / undated records</th><th>Reconciliation</th><th>Review</th></tr></thead><tbody>'+rows.map(r=>'<tr><td>'+escapeHtml(r.currency+' / '+r.taxBasis)+'</td><td>'+escapeHtml(fmtExecutive(r.sourceAggregate)+' · '+humanizeKey(r.sourceState))+'</td><td>'+escapeHtml(fmtExecutive(r.datedApprovedAmount))+'</td><td>'+escapeHtml(r.datedApprovedCount+' / '+r.futureCount+' / '+r.undatedCount)+'</td><td>'+escapeHtml(humanizeKey(r.state))+'</td><td>'+managementModuleLink("variations-change","Open dated ledger")+'</td></tr>').join("")+'</tbody></table></div>';
}
    ...
    return '<div class="planning-view management-view master-dashboard-view">'+
      ...
      experienceDisclosure("Specialist coverage and evidence",'<div class="management-two-column">'+
        ...
        managementPanel("Evidence Snapshot","The dashboard is a view of the current governed project evidence, not an independent data store.",planningKpis([
          ...
          ["Checked specialist views",r.ready??0,"calculation, evidence and consistency gates"],
  ...
  if(key==="master-control-programme"){
    ...
    return '<div class="planning-view management-view master-control-view">'+
      managementPanel("Programme control","Current programme, controlled baseline and project structure.",planningKpis([
        ...
        ["Controlled baseline",r.baselineLabel?planningRevisionLabel(r.baselineLabel):"Not established",r.baselineRevisionId?"controlled revision; full identity in evidence detail":"no governed baseline",r.baselineRevisionId?"success":"warning"],
      ...
      managementPanel("Governance Readiness","Publication is an output; its absence does not invalidate established source facts.",renderManagementEvidenceGaps(data.governanceGaps||[])+renderManagementConsistency(data.consistency))+
...
function renderClaimsReporting(r){
  if(!r)return "";
...
function renderModuleBasis(data,detail=false){
  ...
  const populations=contract?Object.entries(contract.populations||{}):[];
...
function projectCard(p){
  ...
  const eot=p.approvedEotDays===null||p.approvedEotDays===undefined?"Source-approved EOT not established":fmt(p.approvedEotDays)+" gross source-approved days through DD; further adjustment not inferred";
  const forecastLabel=p.forecastAuthority==="scenario"?"CMeng forecast scenario":p.forecastAuthority==="deterministic"?"CMeng calculated forecast":"CMeng forecast not established";
  const contractLabel=p.officialCompletionIso?(p.contractualCompletionState==="established"?"Official contract: ":"Contract source / review: ")+official:"Contract completion not established";
  const adjustmentLabel=p.furtherAdjustedCompletionIso?"Further adjusted: "+planningShortDate(p.furtherAdjustedCompletionIso):"Further adjustment not established";
  ...
  return '<article class="portfolio-project">'+
    '<div class="portfolio-project-main">'+
      ...
      '<div class="portfolio-project-metric"><span>Management</span><strong>'+escapeHtml(managementCount===null?"Not established":fmt(managementCount)+" action"+(managementCount===1?"":"s"))+'</strong><small>'+escapeHtml(p.commercialCurrencyCount?fmt(p.commercialCurrencyCount)+" commercial currenc"+(p.commercialCurrencyCount===1?"y":"ies"):"No commercial position")+'</small></div>'+
