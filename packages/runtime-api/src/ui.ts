...
async function deleteProjectDocument(documentId,filename){
  const ok=confirm('Delete "'+filename+'"?\n\nThis removes the document from the project record and updates the project position. If it is the current programme, BOQ or contract, CMeng will rebuild the remaining project basis.');
  if(!ok)return;
  setBusy("Deleting project document");
  try{
    ...
    el("globalStatus").textContent='Deleted '+filename+(result.deleted?.replacementDocumentId?" · previous document restored as current":"");
    await refresh(false);
  }catch(e){
