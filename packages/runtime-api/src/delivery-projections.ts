...
function buildDelivery(state:ProjectRuntimeState){
 ...
 const controlledSubsetValue=(rows:Array<{amount:number|null}>)=>!controlledBoq?null:rows.length?sumKnown(rows.map(r=>r.amount)):0;
 const currencyGroups=[...new Set(boqRows.map(r=>r.currency).filter((c):c is string=>!!c))].map(currency=>{const rows=boqRows.filter(r=>r.currency===currency);return {currency,totalValue:controlledBoq?sumKnown(rows.map(r=>r.amount)):null,mappedValue:controlledSubsetValue(rows.filter(r=>mapped.has(r.itemId))),unmappedValue:controlledSubsetValue(rows.filter(r=>!mapped.has(r.itemId)))};});
 ...
 const weatherRows=registerRows.filter(r=>r.kind==='weather').map(r=>{const source=byId.get(r.recordId)!,eventStart=date(source,'event start'),eventEnd=date(source,'event end'),current=isOnDate(eventStart,dataDateIso)&&isOnDate(eventEnd,dataDateIso);return {...r,eventStart,eventEnd,durationHours:current?numeric(source,'duration hours'):null,recordedWorkingImpactHours:current?numeric(source,'recorded working impact hours'):null,claimIds:source.links.claimIds,causationBasis:'Recorded site facts only. Delay causation and entitlement remain in the existing claims authority.'};});
 for(const r of registerRows){const source=byId.get(r.recordId)!;if(r.overdue)add('RECORD_OVERDUE',source,(r.reference??r.kind)+' is overdue at the programme Data Date.','Review the required response or completion action and its dated evidence.');if(['failed','rejected','retest required'].includes(r.currentStatus))add('OUTCOME_REQUIRES_ACTION',source,(r.reference??r.kind)+' has a dated '+r.currentStatus+' outcome.','Review rectification, retesting and the applicable acceptance evidence.');}
 for(const r of permitRows)if(r.permitStatus==='expired')add('PERMIT_EXPIRED',byId.get(r.recordId)!,(r.reference??'Permit')+' expired before the programme Data Date.','Confirm renewal and affected workfront restrictions.');
 for(const r of readiness)if(r.state==='blocked')add('READINESS_BLOCKED',byId.get(r.recordId)!,(r.reference??r.kind)+' has a confirmed blocking prerequisite.','Open the gate detail and resolve the dated blocker before releasing this scope.');
...
export function deliveryModule(state:ProjectRuntimeState,key:string):ModuleRuntimeResult{
 ...
 const p=deliveryPosition(state),kind=definition[3],title=definition[1],riskPage=key==='delivery-risks',risk=p.existingAuthorities.risk;
 ...
 const hasRows=riskPage?risk.state!=='missing':records.length>0||rows.length>0;const pending=records.filter(r=>['extracted_candidate','working','conflicted','stale'].includes(r.state));
 ...
 if(key==='delivery-quality'||key==='delivery-commissioning')metrics=[...metrics.slice(0,3),metric('Pass / acceptance rate',summary.passRatePercent,'%',summary.basis),metric('Unknown outcomes',rows.length?summary.unknownOutcomeCount:null)];
 ...
 if(hasRows&&p.dataDateIso&&!pending.length){
  if(key==='construction-readiness'||key==='procurement-readiness')managementPosition=rows.filter(r=>r.state==='ready').length+' ready, '+rows.filter(r=>r.state==='blocked').length+' blocked and '+rows.filter(r=>r.state==='unknown').length+' unresolved in the reviewed scope. '+(population.state==='established'?'The scope population is confirmed.':'The complete scope population still requires confirmation.');
  ...
  else if(!findings.length&&population.state!=='established'&&key!=='delivery-risks')managementPosition='Reviewed records are shown below. Confirm the complete applicable population before relying on totals or percentages.';
 ...
 if(riskPage)managementPosition=risk.state==='missing'?'The project risk register is not established. Add or review risk evidence in Documents; Delivery uses that register and its existing scores.':!p.dataDateIso?'Adopt the current programme to establish the risk reporting date. Supplied risk evidence remains available in Documents.':risk.sourceRecordCount+' risk records are supplied: '+risk.currentRecordCount+' current, '+risk.futureRecordCount+' after the Data Date and '+risk.undatedRecordCount+' without a reporting date. '+(risk.unknownStatusCount?risk.unknownStatusCount+' current risk statuses need dated evidence. ':'')+(risk.validation.state==='conflicted'?'Supplied risk ratings conflict; review the existing risk assessment. ':'')+(pending.length?pending.length+' linked Delivery records await review; these are not additional risks.':'');
 const ready=riskPage?risk.complete&&risk.validation.state!=='conflicted'&&!!p.dataDateIso:hasRows&&!unread&&!!p.dataDateIso&&pending.length===0&&findings.length===0&&population.state==='established'&&metrics.every(m=>m.value!==null);
 ...
 return {key,status:!hasRows?'blocked':ready?'ready':'partial',reason:managementPosition,dependencies:['controlled programme','applicable Delivery records'],scheduleAuthorityReview:p.scheduleAuthorityReview,
  data:{projectionKey:'delivery',deliveryPage:key,projectId:state.projectId,projectVersion:state.version,dataDateIso:p.dataDateIso,programmeLabel:p.programmeLabel,programmeRevisionId:p.programmeRevisionId,title,kind,managementPosition,rows:rows.map(r=>({...r,projectId:state.projectId})),metrics,findings,
