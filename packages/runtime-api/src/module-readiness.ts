...
export function enforceModuleReadiness(result: ModuleRuntimeResult, consistency: Pick<CrossModuleCertification, 'state' | 'failedCheckIds' | 'checkCount'> & Partial<Pick<CrossModuleCertification,'checks'>>): ModuleRuntimeResult {
  const issueAssessment = assessModuleIssues(result, consistency);
  const data = result.data as any;
  if (!data || typeof data !== 'object') return {...result, issueAssessment};
  ...
  const checked = result.status === 'ready' && failures.length === 0 && issueAssessment.issues.length === 0;
